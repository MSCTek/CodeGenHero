...
using static CodeGenHero.Logging.Enums;
...
namespace CodeGenHero.Logging.Log4net
{
	public class Log4NetLoggingService : ILoggingService //, log4net.ILog
	{
		...
		public Guid Warn(string message, int logMessageType, Exception ex, string userName = null, string clientIPAddress = null,
			[CallerMemberName] string methodName = null, [CallerFilePath] string sourceFile = null, [CallerLineNumber] int lineNumber = 0,
			Decimal? executionTimeInMilliseconds = null, int? httpResponseStatusCode = null, string url = null)
		{
			var logGuid = System.Guid.NewGuid();
		...
		private ILog SetCustomContextProperties(int logMessageType,
			...
			Decimal? executionTimeInMilliseconds, int? httpResponseStatusCode, string url)
		{
			...
			log4net.LogicalThreadContext.Properties["LogMessageTypeID"] = logMessageType;
		...
		[Conditional("DEBUG")]
		private void WriteDebugDiagnostics(string message, int logMessageType, Exception ex, string userName, string clientIPAddress,
