...
public partial class CompaniesBBController : BBBaseApiController
{
	...
	= new GenericFactory<entBB.Company, dtoBB.Company>();
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableCompany();
		...
		[HttpGet]
		[VersionedRoute(template: "Companies/{companyId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid companyId)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableCompany();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, companyId);
	...
	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entBB.Company> dbItems, System.Guid companyId);
