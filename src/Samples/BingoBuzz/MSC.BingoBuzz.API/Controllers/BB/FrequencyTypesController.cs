...
public partial class FrequencyTypesBBController : BBBaseApiController
{
	...
	= new GenericFactory<entBB.FrequencyType, dtoBB.FrequencyType>();
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableFrequencyType();
		...
		[HttpGet]
		[VersionedRoute(template: "FrequencyTypes/{frequencyTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int frequencyTypeId)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableFrequencyType();
				RunCustomLogicAfterGetQueryableByPK(ref qryItem, frequencyTypeId);
	...
	partial void RunCustomLogicAfterGetQueryableByPK(ref IQueryable<entBB.FrequencyType> dbItems, int frequencyTypeId);
