	...
	= new GenericFactory<entBB.BingoInstanceEvent, dtoBB.BingoInstanceEvent>();
		...
		public async Task<IHttpActionResult> Delete(System.Guid bingoInstanceEventId)
		{
			try
			{
				...
				var result = Repo.DeleteBingoInstanceEvent(bingoInstanceEventId);
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableBingoInstanceEvent().AsNoTracking();
		...
		[HttpGet]
		[VersionedRoute(template: "BingoInstanceEvents/{bingoInstanceEventId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid bingoInstanceEventId, int numChildLevels = 0)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableBingoInstanceEvent().AsNoTracking();
				RunCustomLogicOnGetQueryableByPK(ref qryItem, bingoInstanceEventId, numChildLevels);

				var dbItem = qryItem.Where(x => x.BingoInstanceEventId == bingoInstanceEventId).FirstOrDefault();
		...
		public async Task<IHttpActionResult> Patch(System.Guid bingoInstanceEventId, [FromBody] JsonPatchDocument<dtoBB.BingoInstanceEvent> patchDocument)
		{
			try
			{
				...
				var dbItem = Repo.GetBingoInstanceEvent(bingoInstanceEventId);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		partial void RunCustomLogicAfterInsert(entBB.BingoInstanceEvent newDBItem);

		partial void RunCustomLogicOnGetQueryableByPK(ref IQueryable<entBB.BingoInstanceEvent> qryItem, System.Guid bingoInstanceEventId, int numChildLevels);
		...
		// {
			// if (numChildLevels > 0)
			// {
				// qryItem = qryItem.Include(x => x.BingoInstance).AsNoTracking()
				// .Include(x => x.BingoInstanceContent).AsNoTracking()
