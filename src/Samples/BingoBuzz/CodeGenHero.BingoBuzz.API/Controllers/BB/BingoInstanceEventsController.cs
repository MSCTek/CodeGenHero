...
{
	public partial class BingoInstanceEventsBBController : BBBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "BingoInstanceEventsBBList";
			private const int maxPageSize = 100;

			private GenericFactory<entBB.BingoInstanceEvent, dtoBB.BingoInstanceEvent> _factory 
				= new GenericFactory<entBB.BingoInstanceEvent, dtoBB.BingoInstanceEvent>();

		public BingoInstanceEventsBBController() : base()
		{
		}

		public BingoInstanceEventsBBController(ILoggingService log, IBBRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "BingoInstanceEvents/{bingoInstanceEventId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid bingoInstanceEventId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BingoInstanceEventAsync(bingoInstanceEventId);
		...
		[HttpGet]
		[VersionedRoute(template: "BingoInstanceEvents", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BingoInstanceEvent().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entBB.BingoInstanceEvent).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BingoInstanceEvents/{bingoInstanceEventId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid bingoInstanceEventId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BingoInstanceEventAsync(bingoInstanceEventId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, bingoInstanceEventId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BingoInstanceEvents/{bingoInstanceEventId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid bingoInstanceEventId, [FromBody] JsonPatchDocument<dtoBB.BingoInstanceEvent> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BingoInstanceEventAsync(bingoInstanceEventId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.BingoInstanceEventId = bingoInstanceEventId;
		...
		[HttpPost]
		[VersionedRoute(template: "BingoInstanceEvents", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoBB.BingoInstanceEvent dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BingoInstanceEventId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BingoInstanceEvents/{bingoInstanceEventId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid bingoInstanceEventId, [FromBody] dtoBB.BingoInstanceEvent dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoInstanceEvent newDBItem, ref IRepositoryActionResult<entBB.BingoInstanceEvent> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoInstanceEvent updatedDBItem, ref IRepositoryActionResult<entBB.BingoInstanceEvent> result);

		partial void RunCustomLogicAfterUpdatePut(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoInstanceEvent updatedDBItem, ref IRepositoryActionResult<entBB.BingoInstanceEvent> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entBB.BingoInstanceEvent dbItem, System.Guid bingoInstanceEventId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entBB.BingoInstanceEvent> dbItems, ref List<string> filterList);
	}
}