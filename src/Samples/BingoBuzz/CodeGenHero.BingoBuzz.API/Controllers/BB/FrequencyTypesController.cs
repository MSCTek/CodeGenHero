	...
	public partial class FrequencyTypesBBController : BBBaseApiController
	{
		...
		public async Task<IHttpActionResult> Delete(int frequencyTypeId)
		{
			try
			{
				...
				var result = await Repo.Delete_FrequencyTypeAsync(frequencyTypeId);
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_FrequencyType().AsNoTracking();
		...
		[HttpGet]
		[VersionedRoute(template: "FrequencyTypes/{frequencyTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int frequencyTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_FrequencyTypeAsync(frequencyTypeId, numChildLevels);
		...
		public async Task<IHttpActionResult> Patch(int frequencyTypeId, [FromBody] JsonPatchDocument<dtoBB.FrequencyType> patchDocument)
		{
			try
			{
				...
				var dbItem = await Repo.Get_FrequencyTypeAsync(frequencyTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		partial void RunCustomLogicAfterInsert(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.FrequencyType newDBItem, ref IRepositoryActionResult<entBB.FrequencyType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.FrequencyType updatedDBItem, ref IRepositoryActionResult<entBB.FrequencyType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.FrequencyType updatedDBItem, ref IRepositoryActionResult<entBB.FrequencyType> result);
