	...
	= new GenericFactory<entBB.MeetingSchedule, dtoBB.MeetingSchedule>();
		...
		public async Task<IHttpActionResult> Delete(System.Guid meetingScheduleId)
		{
			try
			{
				...
				var result = Repo.DeleteMeetingSchedule(meetingScheduleId);
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableMeetingSchedule().AsNoTracking();
		...
		[HttpGet]
		[VersionedRoute(template: "MeetingSchedules/{meetingScheduleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid meetingScheduleId, int numChildLevels = 0)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableMeetingSchedule().AsNoTracking();
				RunCustomLogicOnGetQueryableByPK(ref qryItem, meetingScheduleId, numChildLevels);

				var dbItem = qryItem.Where(x => x.MeetingScheduleId == meetingScheduleId).FirstOrDefault();
		...
		public async Task<IHttpActionResult> Patch(System.Guid meetingScheduleId, [FromBody] JsonPatchDocument<dtoBB.MeetingSchedule> patchDocument)
		{
			try
			{
				...
				var dbItem = Repo.GetMeetingSchedule(meetingScheduleId);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		partial void RunCustomLogicAfterInsert(entBB.MeetingSchedule newDBItem);

		partial void RunCustomLogicOnGetQueryableByPK(ref IQueryable<entBB.MeetingSchedule> qryItem, System.Guid meetingScheduleId, int numChildLevels);
