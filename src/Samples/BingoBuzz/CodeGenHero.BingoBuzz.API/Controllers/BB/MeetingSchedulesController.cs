...
{
	public partial class MeetingSchedulesBBController : BBBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "MeetingSchedulesBBList";
			private const int maxPageSize = 100;

			private GenericFactory<entBB.MeetingSchedule, dtoBB.MeetingSchedule> _factory 
				= new GenericFactory<entBB.MeetingSchedule, dtoBB.MeetingSchedule>();

		public MeetingSchedulesBBController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "MeetingSchedules/{meetingScheduleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid meetingScheduleId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_MeetingScheduleAsync(meetingScheduleId);
		...
		[HttpGet]
		[VersionedRoute(template: "MeetingSchedules", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_MeetingSchedule().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entBB.MeetingSchedule).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "MeetingSchedules/{meetingScheduleId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid meetingScheduleId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_MeetingScheduleAsync(meetingScheduleId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, meetingScheduleId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "MeetingSchedules/{meetingScheduleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid meetingScheduleId, [FromBody] JsonPatchDocument<dtoBB.MeetingSchedule> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_MeetingScheduleAsync(meetingScheduleId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.MeetingScheduleId = meetingScheduleId;
		...
		[HttpPost]
		[VersionedRoute(template: "MeetingSchedules", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoBB.MeetingSchedule dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.MeetingScheduleId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "MeetingSchedules/{meetingScheduleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid meetingScheduleId, [FromBody] dtoBB.MeetingSchedule dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.MeetingSchedule newDBItem, ref IRepositoryActionResult<entBB.MeetingSchedule> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.MeetingSchedule updatedDBItem, ref IRepositoryActionResult<entBB.MeetingSchedule> result);

		partial void RunCustomLogicAfterUpdatePut(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.MeetingSchedule updatedDBItem, ref IRepositoryActionResult<entBB.MeetingSchedule> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entBB.MeetingSchedule dbItem, System.Guid meetingScheduleId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entBB.MeetingSchedule> dbItems, ref List<string> filterList);
	}
}