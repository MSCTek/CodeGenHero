	...
	public partial class BingoContentsBBController : BBBaseApiController
	{
		...
		public async Task<IHttpActionResult> Delete(System.Guid bingoContentId)
		{
			try
			{
				...
				var result = await Repo.Delete_BingoContentAsync(bingoContentId);
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BingoContent().AsNoTracking();
		...
		[HttpGet]
		[VersionedRoute(template: "BingoContents/{bingoContentId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid bingoContentId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BingoContentAsync(bingoContentId, numChildLevels);
		...
		public async Task<IHttpActionResult> Patch(System.Guid bingoContentId, [FromBody] JsonPatchDocument<dtoBB.BingoContent> patchDocument)
		{
			try
			{
				...
				var dbItem = await Repo.Get_BingoContentAsync(bingoContentId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		partial void RunCustomLogicAfterInsert(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoContent newDBItem, ref IRepositoryActionResult<entBB.BingoContent> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoContent updatedDBItem, ref IRepositoryActionResult<entBB.BingoContent> result);

		partial void RunCustomLogicAfterUpdatePut(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoContent updatedDBItem, ref IRepositoryActionResult<entBB.BingoContent> result);
