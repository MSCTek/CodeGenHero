...
namespace CodeGenHero.BingoBuzz.API.Controllers.BB
{
	...
	{
			private const string GET_LIST_ROUTE_NAME = "BingoInstancesBBList";
			private const int maxPageSize = 100;

			private GenericFactory<entBB.BingoInstance, dtoBB.BingoInstance> _factory 
				= new GenericFactory<entBB.BingoInstance, dtoBB.BingoInstance>();

		public BingoInstancesBBController() : base()
		{
		}

		public BingoInstancesBBController(ILoggingService log, IBBRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "BingoInstances/{bingoInstanceId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid bingoInstanceId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BingoInstanceAsync(bingoInstanceId);
		...
		[HttpGet]
		[VersionedRoute(template: "BingoInstances", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BingoInstance().AsNoTracking();
		...
		[HttpGet]
		[VersionedRoute(template: "BingoInstances/{bingoInstanceId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid bingoInstanceId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BingoInstanceAsync(bingoInstanceId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, bingoInstanceId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BingoInstances/{bingoInstanceId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid bingoInstanceId, [FromBody] JsonPatchDocument<dtoBB.BingoInstance> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BingoInstanceAsync(bingoInstanceId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.BingoInstanceId = bingoInstanceId;
		...
		[HttpPost]
		[VersionedRoute(template: "BingoInstances", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoBB.BingoInstance dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "BingoInstances/{bingoInstanceId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid bingoInstanceId, [FromBody] dtoBB.BingoInstance dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoInstance newDBItem, ref IRepositoryActionResult<entBB.BingoInstance> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoInstance updatedDBItem, ref IRepositoryActionResult<entBB.BingoInstance> result);

		partial void RunCustomLogicAfterUpdatePut(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.BingoInstance updatedDBItem, ref IRepositoryActionResult<entBB.BingoInstance> result);
