	...
	= new GenericFactory<entBB.BingoInstance, dtoBB.BingoInstance>();
		...
		public async Task<IHttpActionResult> Delete(System.Guid bingoInstanceId)
		{
			try
			{
				...
				var result = Repo.DeleteBingoInstance(bingoInstanceId);
		...
		[HttpGet]
		[VersionedRoute(template: "BingoInstances/{bingoInstanceId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid bingoInstanceId, int numChildLevels = 0)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableBingoInstance().AsNoTracking();
				RunCustomLogicOnGetQueryableByPK(ref qryItem, bingoInstanceId, numChildLevels);

				var dbItem = qryItem.Where(x => x.BingoInstanceId == bingoInstanceId).FirstOrDefault();
		...
		public async Task<IHttpActionResult> Patch(System.Guid bingoInstanceId, [FromBody] JsonPatchDocument<dtoBB.BingoInstance> patchDocument)
		{
			try
			{
				...
				var dbItem = Repo.GetBingoInstance(bingoInstanceId);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		partial void RunCustomLogicAfterInsert(entBB.BingoInstance newDBItem);
		...
		// {
			// if (numChildLevels > 0)
			// {
				// qryItem = qryItem.Include(x => x.BingoInstanceStatusType).AsNoTracking()
