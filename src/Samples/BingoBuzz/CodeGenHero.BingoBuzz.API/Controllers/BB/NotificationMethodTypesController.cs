	...
	public partial class NotificationMethodTypesBBController : BBBaseApiController
	{
		...
		public async Task<IHttpActionResult> Delete(int notificationMethodTypeId)
		{
			try
			{
				...
				var result = await Repo.Delete_NotificationMethodTypeAsync(notificationMethodTypeId);
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_NotificationMethodType().AsNoTracking();
		...
		[HttpGet]
		[VersionedRoute(template: "NotificationMethodTypes/{notificationMethodTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int notificationMethodTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_NotificationMethodTypeAsync(notificationMethodTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, notificationMethodTypeId, numChildLevels);
		...
		public async Task<IHttpActionResult> Patch(int notificationMethodTypeId, [FromBody] JsonPatchDocument<dtoBB.NotificationMethodType> patchDocument)
		{
			try
			{
				...
				var dbItem = await Repo.Get_NotificationMethodTypeAsync(notificationMethodTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		partial void RunCustomLogicAfterInsert(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.NotificationMethodType newDBItem, ref IRepositoryActionResult<entBB.NotificationMethodType> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.NotificationMethodType updatedDBItem, ref IRepositoryActionResult<entBB.NotificationMethodType> result);

		partial void RunCustomLogicAfterUpdatePut(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.NotificationMethodType updatedDBItem, ref IRepositoryActionResult<entBB.NotificationMethodType> result);
