	...
	public partial class NotificationRulesBBController : BBBaseApiController
	{
		...
		public async Task<IHttpActionResult> Delete(System.Guid notificationRuleId)
		{
			try
			{
				...
				var result = Repo.DeleteNotificationRule(notificationRuleId);
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryableNotificationRule().AsNoTracking();
		...
		[HttpGet]
		[VersionedRoute(template: "NotificationRules/{notificationRuleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid notificationRuleId, int numChildLevels = 0)
		{
			try
			{
				base.OnActionExecuting();

				var qryItem = Repo.GetQueryableNotificationRule().AsNoTracking();
				RunCustomLogicOnGetQueryableByPK(ref qryItem, notificationRuleId, numChildLevels);

				var dbItem = qryItem.Where(x => x.NotificationRuleId == notificationRuleId).FirstOrDefault();
		...
		public async Task<IHttpActionResult> Patch(System.Guid notificationRuleId, [FromBody] JsonPatchDocument<dtoBB.NotificationRule> patchDocument)
		{
			try
			{
				...
				var dbItem = Repo.GetNotificationRule(notificationRuleId);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		partial void RunCustomLogicAfterInsert(entBB.NotificationRule newDBItem);

		partial void RunCustomLogicOnGetQueryableByPK(ref IQueryable<entBB.NotificationRule> qryItem, System.Guid notificationRuleId, int numChildLevels);
