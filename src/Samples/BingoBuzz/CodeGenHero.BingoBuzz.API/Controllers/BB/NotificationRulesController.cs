...
{
	public partial class NotificationRulesBBController : BBBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "NotificationRulesBBList";
			private const int maxPageSize = 100;

			private GenericFactory<entBB.NotificationRule, dtoBB.NotificationRule> _factory 
				= new GenericFactory<entBB.NotificationRule, dtoBB.NotificationRule>();

		public NotificationRulesBBController() : base()
		{
		}

		public NotificationRulesBBController(ILoggingService log, IBBRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "NotificationRules/{notificationRuleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid notificationRuleId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_NotificationRuleAsync(notificationRuleId);
		...
		[HttpGet]
		[VersionedRoute(template: "NotificationRules", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_NotificationRule().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entBB.NotificationRule).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "NotificationRules/{notificationRuleId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid notificationRuleId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_NotificationRuleAsync(notificationRuleId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, notificationRuleId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "NotificationRules/{notificationRuleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid notificationRuleId, [FromBody] JsonPatchDocument<dtoBB.NotificationRule> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_NotificationRuleAsync(notificationRuleId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.NotificationRuleId = notificationRuleId;
		...
		[HttpPost]
		[VersionedRoute(template: "NotificationRules", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoBB.NotificationRule dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.NotificationRuleId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "NotificationRules/{notificationRuleId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid notificationRuleId, [FromBody] dtoBB.NotificationRule dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.NotificationRule newDBItem, ref IRepositoryActionResult<entBB.NotificationRule> result);

		partial void RunCustomLogicAfterUpdatePatch(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.NotificationRule updatedDBItem, ref IRepositoryActionResult<entBB.NotificationRule> result);

		partial void RunCustomLogicAfterUpdatePut(ref CodeGenHero.BingoBuzz.Repository.Entities.BB.NotificationRule updatedDBItem, ref IRepositoryActionResult<entBB.NotificationRule> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entBB.NotificationRule dbItem, System.Guid notificationRuleId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entBB.NotificationRule> dbItems, ref List<string> filterList);
	}
}